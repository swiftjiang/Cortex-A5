...
BYTE CHIDDevice::QueueInterruptReadData(DWORD TimeoutMSec)
{
    ...
    // Get max number of reports in the HIDAPI interrupt report queue
    for (DWORD i = 0; i < GetMaxReportRequest(); i++)
    {
        memset(buffer, 0x00, bufferLength);
        
        // Non-blocking read
        result = hid_read_timeout(m_handle, buffer, bufferLength, TimeoutMSec);
...
BYTE CHIDDevice::SetOutputReport_Interrupt(BYTE* buffer, DWORD bufferSize)
{
    ...
    // Make sure that the pointer is valid
    if (buffer && bufferSize <= (DWORD) m_outputReportBufferLength)
    {
        // Make sure the device is opened
        if (m_deviceOpened)
        {
        ...
        *bytesReturned = 0;
        ...
        if (m_deviceOpened)
        {
            ...
            do
            {
            	...
            	// Read data from the interrupt pipe and enqueue
            	BYTE queueStatus = QueueInterruptReadData(getReportTimeout - elapsTime);

            	// Return immediately if there was an error
            	if (queueStatus != HID_DEVICE_SUCCESS && queueStatus != HID_DEVICE_TRANSFER_TIMEOUT)
            	{
...
DWORD CHIDDevice::GetMaxReportRequest()
{
    return m_maxReportRequest;
}
